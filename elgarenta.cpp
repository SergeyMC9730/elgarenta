...
{
    if (!inst->firstr)
        glDeleteTextures(1, &inst->rendered_image.texture);

    // printf("RENDER FRAME\n");

    char *render_buffer = (char *)inst->fb;
    int ii = 0, i = 0;
    while (ii < (inst->renderw * inst->renderh * 3))
        render_buffer[ii++] = 0;
    ...
    while (i < MAX_JOBS)
    {
        ...
        if (inst->render_jobs[i].used)
        {
            ...
            case JOB_FBPIPE:
            {
                int i1 = inst->render_jobs[i].x;
                int i2 = inst->render_jobs[i].y;
                int i1_1 = 0;
                int i2_1 = 0;
                char *framebuffer = (char *)inst->render_jobs[i].arg0;
                while ((i1 < (inst->render_jobs[i].x + inst->render_jobs[i].sx)) && (inst->render_jobs[i].x < inst->renderw) && (inst->render_jobs[i].y < inst->renderh))
                {
                    if (i1 < inst->renderw)
                    {
                        while (i2 < (inst->render_jobs[i].y + inst->render_jobs[i].sy))
                        {
                            if (i2 < inst->renderh)
                            {
                                if (inst->render_jobs[i].blend)
                                {
                                    inst->fb[i1 * inst->renderw * 3 + i2 * 3 + 0] += framebuffer[i1_1 * inst->render_jobs[i].sx * 3 + i2_1 * 3 + 0];
                                    inst->fb[i1 * inst->renderw * 3 + i2 * 3 + 1] += framebuffer[i1_1 * inst->render_jobs[i].sx * 3 + i2_1 * 3 + 1];
                                    inst->fb[i1 * inst->renderw * 3 + i2 * 3 + 2] += framebuffer[i1_1 * inst->render_jobs[i].sx * 3 + i2_1 * 3 + 2];

                                    i1_1++;

                                    if (i1_1 == inst->render_jobs[i].sx) {
                                        i1_1 = 0;
                                        i2_1++;
                                    }
                                }
...
elgarenta::job_t *elgarenta::create_job(elgarenta::job_t j, elgarenta::instance_t *inst)
{
    ...
    while (ii < MAX_JOBS)
    {
        if (!inst->render_jobs[ii].used)
        {
            ...
            inst->updating = false;
            printf("[Elgarenta] Created job %d:%d\n", ii, id);
            return &inst->render_jobs[ii];
        }
        ii++;
    }
